...
# Inferência da estrutura da Rede Causal usando o algoritmo **mmpc**
```{r echo=TRUE}
...
```{r}
graphviz.plot(bn_mmpc, shape = 'rectangle', main = "Algo = mmpc. DATA = full")
```



```{r}
graphviz.plot(bn_mmpc_covid, shape = 'rectangle', main = "Algo = mmpc. DATA = COVID")
```



```{r}
graphviz.plot(bn_mmpc_outras, shape='rectangle', main = "Algo = mmpc. DATA = Outras")
```
...
graphviz.plot(bn_mmhc, shape = 'rectangle', main = "Data = full, ALGO = mmhc")

graphviz.plot(bn_mmhc_outras, shape = 'rectangle', main = "Data = outras, ALGO = mmhc")

graphviz.plot(bn_mmhc_covid, shape = 'rectangle', main = "Data = COVID, ALGO = mmhc")
```
...
## Inferência da estrutura da Rede Causal usando o algoritmo **iamb**
```{r iamb, echo=TRUE}
suppressWarnings(bn_iamb <- iamb(x_no_na))
graphviz.plot(bn_iamb, shape = 'rectangle')
```

# Inferência da estrutura da Rede Causal usando o algoritmo **gs**
```{r gs, echo=TRUE}
suppressWarnings(bn_gs <- gs(x_no_na))
graphviz.plot(bn_gs, shape = 'rectangle')
```

# Inferência da estrutura da Rede Causal usando o algoritmo **hc**
```{r hc, echo=TRUE}
suppressWarnings(bn_hc <- hc(x_no_na))
suppressWarnings(bn_hc_outras <- hc(srag_outras))
suppressWarnings(bn_hc_covid <- hc(srag_covid))
graphviz.plot(bn_hc, shape = 'rectangle', main = "Algo = hc DATA = full")
graphviz.plot(bn_hc_covid, shape = 'rectangle', main = "Algo = hc DATA = COVID")
graphviz.plot(bn_hc_outras, shape = 'rectangle', main = "Algo = hc DATA = OUTRAS")
```

# Ajustamento da estrutura do algoritmo *HC* com os dados
```{r fit, echo=FALSE}
...
## Descrição estatística da rede para a variável EVOLUCAO DATA = full
```{r echo=FALSE}
fitted_hc$EVOLUCAO
``` 
...
```{r echo=FALSE}
suppressWarnings(bn.fit.barchart(fitted_hc$EVOLUCAO, xlab = "hc Probabilidades FULL", ylab = "EVOLUCAO FULL"))

suppressWarnings(bn.fit.barchart(fitted_hc_covid$EVOLUCAO, xlab = "hc Probabilidades COVID", ylab = "EVOLUCAO COVID"))

suppressWarnings(bn.fit.barchart(fitted_hc_outras$EVOLUCAO, xlab = "hc Probabilidades OUTRAS", ylab = "EVOLUCAO OUTRAS"))
...
# Ajustamento da estrutura do algoritmo *MMHC* com os dados
## Descrição estatística da rede para a variável EVOLUCAO ALGO = mmhc
```{r echo=FALSE}
...
suppressWarnings(bn.fit.barchart(fitted_mmhc$EVOLUCAO, xlab = "mmhc Probabilidades FULL", ylab = "EVOLUCAO FULL"))

suppressWarnings(bn.fit.barchart(fitted_mmhc_covid$EVOLUCAO, xlab = "mmhc Probabilidades COVID", ylab = "EVOLUCAO COVID"))

suppressWarnings(bn.fit.barchart(fitted_mmhc_outras$EVOLUCAO, xlab = "mmhc Probabilidades OUTRAS", ylab = "EVOLUCAO OUTRAS"))
