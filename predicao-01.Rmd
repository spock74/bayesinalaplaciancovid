...
```{r}
amostra.sim <- na.omit(amostra.simulada.2)
rede.sim <- mmhc(amostra.simulada.2)
rede.real <- mmhc(amostra.real.2)
```
...
## Aprender rede com 300 iteracoes de bootstrap
```{r echo=TRUR}
...
```{r}
fitt.1 <- bn.fit(rede.media.sim, amostra.sim)
...
100 - (prb - prob.uti.1)/prb*100
```

```{r}
xr <- amostra.90pc.seed.42 %>% filter(
                                     SATURACAO == 1, 
                                     #UTI == 1, 
                                     FEBRE == 1, 
                                     #DIABETES == 2, 
...
nrow(xr)
ar <- filter(xr, EVOLUCAO == 2)
nrow(ar)

prbr <- nrow(ar)/nrow(xr)
prbr
```

## Predicção, desempenho
```{r}
100 - (prbr - prb)/prbr*100

100 - (prb - prob.uti.1)/prbr*100
```
