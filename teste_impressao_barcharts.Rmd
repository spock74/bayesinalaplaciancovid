...
```{r echo=FALSE}
#par(mfrow=c(2,2))
graphviz.plot(bn1, shape='rectangle', main = 'Amostra de 40%')
...
#par(mfrow=c(2,2))
graphviz.chart(fitted.1, type = "barprob", col = "darkblue", bg = "azure", bar.col = "darkblue", main = 'Amostra de 40%')
...
strength.plot(avg.diff, str.diff, shape = "rectangle", main = "Iterações = 50, Amostra = 40%")
```
...
```{r}
par(mfrow = c(1, 2))
graphviz.compare(avg.diff, bn1, shape = "rectangle", main = c("DAG médio. Amostra = 40%", "DAG único. Amostra = 40%"))
```
...
strength.plot(avg.diff, str.diff, shape = "rectangle", main = "Iterações = 50. Amostra = 40%")
strength.plot(avg.simpler, str.diff, shape = "rectangle", main = 'Amostra 40%')
